...
A wizard is ephemeral by default: built for one run, saved to `.agents/scratchpad/wizards/`, deleted when the job's done. Only when the user wants a repeatable setup path that should live in the repo does it move to `scripts/` and get committed, and that's their call to make, not yours.
